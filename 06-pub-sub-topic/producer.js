...
amqp
  ...
  .then(function (conn) {
    ...
    ok = ok.then(async function (ch) {
      ...
      setInterval(async () => {
        await produceMessages(ch, "event.sport.football", exchange);
        await produceMessages(ch, "event.sport.football.barcelona", exchange);
        await produceMessages(ch, "event.weather.paris", exchange);
        await produceMessages(ch, "event.weather.paris.temperature", exchange);
      }, 1000);
    });
    return ok;
  })
